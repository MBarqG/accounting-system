...
@section('content')
    ...
    <form action="{{ route('debt.store') }}" method="POST">
        @csrf
        <div class="mb-4">
            <label for="customer_id" class="block">adding to <span class="font-bold text-xl">{{ $customer->name }}</span></label>
            <select name="customer_id" class="w-full border rounded px-4 py-2 hidden">
